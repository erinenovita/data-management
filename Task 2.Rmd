...
We first create a RMD file which located in the same directory as the folder which contain all the *xlsx* files, which we named *group_assignment*. Since we are dealing with *xlsx*, we need to import packages *readxl* for reading the files.
...
We then want to know their names inside the directory *group_assignment*.

```{r}
all_folder = list.files(path = "group_assignment/", all.files = F, full.names = F)
head(all_folder)
```
...
XL_sheets = list.files(path = paste("group_assignment/","Albania" ,sep = ""), all.files = F, 
...
```{r, message=FALSE}
current_XL = read_excel(paste("group_assignment/","Albania","/",XL_sheets[1] ,sep = ""))  
...
for (folder in all_folder)  #for all the directories in "group_assignment"
{
  XL_sheets = list.files(path = paste("group_assignment/",folder ,sep = ""), 
  ...
  {
    current_XL = read_excel(paste("group_assignment/",folder,"/",XL ,sep = "")) 
